...
def post_stories():
    ...
    if 'story_name' in data:
        if data['story_name'] is not None:
            new_story = row_constructor(Story, data)
        else:
            return create_response(status=422, message='The story_name parameter was invalid')
    else:
...
def put_stories(story_id):
    ...
    if 'story_name' in data:
        if data['story_name'] is not None:
            ...
            missing_params.append('story_name')
